...
// File: Crypto.cpp
// Functionality: Implementation of Crypto class to handle encryption/decryption of messages.
// Notes:
...
char* Crypto::strToCharArr(std::string str)
{
...
uint8_t* Crypto::encrypt_cbc(uint8_t* incomingMessage)
{
    printf("Encrypting the SBD Message\n");

    //char* plaintext = strToCharArr(incomingMessage);
    uint8_t* plaintext = incomingMessage;
  
    // Local Variables:
    uint8_t key[16] = { 0x74, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73, 0x20, 0x74, 0x65, 0x73, 0x74, 0x20, 0x6b, 0x65, 0x79 }; // Key is 16 Bytes (128 bits)
    uint8_t iv[16]  = { 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76 }; // Initialization vector is 16 Bytes (128 bits)
    struct AES_ctx ctx;
    ...
    // Return the plaintext string length without counting null Byte
    ...
    {
        cipherTextLength = strlen((char*)plaintext); // Length of new ciphertext array 
        ciphertext = new uint8_t[(cipherTextLength + 1) * sizeof(plaintext[0])]; 
    ...
    printf("Length of encrypted ciphertext: %d\n", cipherTextLength);
...
{
    printf("Decrypting the SBD Message\n");
...
uint8_t* Crypto::padString(uint8_t* str)
{
    ...
    uint8_t padByte = (uint8_t)(0x5E); // The padding byte: '^' - Change if it causes issues
    uint8_t totalPadding = (uint8_t)(extraPaddingNum); // Placed as the last character of the padding
    int newStrLength = (originalStrLength + extraPaddingNum); // Captures the new padded string length
    ...
    printf("String requires padding - Adding %d Bytes\n", extraPaddingNum);
...
uint8_t* Crypto::removeStringPadding(uint8_t* str)
{
    ...
    int newStrLength = (originalStrLength - (int)removePaddingNum); 
    uint8_t* returnStr;

    // If the last character is a number 1-15 in hex do another check
    if((int)removePaddingNum >= 0 && (int)removePaddingNum <= 15)
    {
        // Start at the end digit and move back the amount of spaces the padding number at the end signals
        int j = (int)removePaddingNum;
        int k = originalStrLength - j; // Location of the first padding Byte according to the last hex value
        ...
        while(i >= k)
        {
            ...
            // If the special padding byte is not matched then return the string as is because it is unpadded
            else
            {
                return str;
            }
        ...
        returnStr = (uint8_t*)malloc((newStrLength) * sizeof(uint8_t) + 1); 
        if(returnStr == NULL)
        {
            exit(1);
        }
