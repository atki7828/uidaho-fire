...
// These are needed so that our python script can access the encryption and decryption
extern "C"
{
    Crypto* Crypto_new()
    {
    ...
    uint8_t* Crypto_encrypt_cbc(Crypto* crypto, uint8_t* str)
    {
        return crypto->encrypt_cbc(str);
    }

    uint8_t* Crypto_decrypt_cbc(Crypto* crypto, uint8_t* str)
    {
        return crypto->decrypt_cbc(str);
    }
}
